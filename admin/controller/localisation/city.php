...
class ControllerLocalisationCity extends Controller {
    ...
    protected function getList() {
        ...
        $results = $this->model_localisation_city->getCites($filter_data);
        ...
        foreach ($results as $result) {
            $data['cites'][] = array(
                ...
                'zone'    => $result['zone'],
                'name'    => $result['name'],
                'edit'    => $this->url->link('localisation/city/edit', 'token=' . $this->session->data['token'] . '&city_id=' . $result['id'] . $url, 'SSL')
            );
        }
        ...
        $data['sort_zone'] = $this->url->link('localisation/city', 'token=' . $this->session->data['token'] . '&sort=z.code' . $url, 'SSL');
...
//        }
        ...
        if (isset($this->request->post['city_name'])) {
            ...
            $data['city_name'] = $this->model_localisation_city->getCityName($this->request->get['city_id']);

        } else {
            $data['city_name'] = array();
        }
    ...
    protected function validateForm() {
        ...
        foreach ($this->request->post['city_name'] as $language_id => $value) {
            if ((utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 64)) {
                $this->error['city_name'][$language_id] = $this->language->get('error_name');
            }
        }
